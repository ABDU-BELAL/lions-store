    ...
    handlers: {
      GET: async ({ params }) => {
        ...
        if (!ALLOWED.has(bucket) || !path || path.includes("..")) {
          return new Response(JSON.stringify({ debug: true, bucket, path, allowed: [...ALLOWED] }), { status: 404 });
        }
        const { data, error } = await supabaseAdmin.storage.from(bucket).download(path);
        if (error || !data) return new Response(JSON.stringify({ debug: true, bucket, path, error: error?.message }), { status: 404 });

...
import {
  ...
  adminBrand1TestConnection, adminBrand1ListProducts, adminSetProductProvider, adminX3TestConnection, adminX3ListProducts, adminYassenTestConnection, adminYassenListProducts, adminSamaTestConnection, adminSamaListProducts,
...
function ProductsTab({ initialCollectionId, onBack }: { initialCollectionId?: string | null; onBack?: () => void } = {}) {
  ...
  const testSama = useServerFn(adminSamaTestConnection);
  ...
  const testConnSama = useMutation({
    ...
    onSuccess: (r) => {
      if (r.ok) toast.success("اتصال Sama يعمل ✓");
      else toast.error("فشل Sama: " + (r.error ?? "تأكد من التوكن والـ IPs"));
    },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  const samaProducts = useQuery({
    queryKey: ["sama-products"],
    queryFn: () => listSama(),
    enabled: !!editing?.id && editing?.provider === "sama",
    staleTime: 5 * 60_000,
  });
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mb-4 flex-wrap">
        ...
        <button onClick={() => testConnSama.mutate()} disabled={testConnSama.isPending} className="rounded-full bg-secondary border border-border px-4 py-2 text-sm font-bold">
          {testConnSama.isPending ? "..." : "اختبار Sama"}
        </button>
        {!initialCollectionId && (
          ...
          <form onClick={(e) => e.stopPropagation()} onSubmit={(e) => { e.preventDefault(); save.mutate(); }} className="w-full max-w-lg bg-card border-gold rounded-2xl p-5 space-y-3 max-h-[90vh] overflow-y-auto">
            ...
            {editing.id ? (
              <div className="rounded-xl border border-gold/40 bg-gold/5 p-3 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <select value={editing.provider} onChange={(e) => setEditing({ ...editing, provider: e.target.value as "" | "brand1" | "x3" | "yassen" | "sama", provider_product_id: "" })} className="rounded-xl bg-secondary px-3 py-2 text-sm">
                ...
                {editing.provider === "yassen" && (
                  <>
                    {yassenProducts.isLoading && <p className="text-xs text-muted-foreground">جاري تحميل منتجات Yassen...</p>}
                    {yassenProducts.data && !yassenProducts.data.ok && (
                      <p className="text-xs text-destructive">تعذر جلب المنتجات: {yassenProducts.data.error ?? "تأكد من التوكن والـ IPs في لوحة Yassen"}</p>
                    )}
                    {yassenProducts.data?.ok && (
                      <select
                        value={editing.provider_product_id}
                        onChange={(e) => setEditing({ ...editing, provider_product_id: e.target.value })}
                        className="w-full rounded-xl bg-secondary px-3 py-2 text-sm"
                      >
                        <option value="">— اختر منتج Yassen —</option>
                        {yassenProducts.data.products.map((bp) => (
                          <option key={bp.id} value={bp.id}>
                            #{bp.id} • {bp.name} {bp.price ? `($${bp.price})` : ""}
                          </option>
                        ))}
                ...
                {editing.provider === "sama" && (
                  <>
                    {samaProducts.isLoading && <p className="text-xs text-muted-foreground">جاري تحميل منتجات Sama...</p>}
                    {samaProducts.data && !samaProducts.data.ok && (
                      <p className="text-xs text-destructive">تعذر جلب المنتجات: {samaProducts.data.error ?? "تأكد من التوكن والـ IPs في لوحة Sama"}</p>
                    )}
                    {samaProducts.data?.ok && (
                      <select
                        value={editing.provider_product_id}
                        onChange={(e) => setEditing({ ...editing, provider_product_id: e.target.value })}
                        className="w-full rounded-xl bg-secondary px-3 py-2 text-sm"
                      >
                        <option value="">— اختر منتج Sama —</option>
                        {samaProducts.data.products.map((bp) => (
                          <option key={bp.id} value={bp.id}>
                            #{bp.id} • {bp.name} {bp.price ? `($${bp.price})` : ""}
                          </option>
                        ))}

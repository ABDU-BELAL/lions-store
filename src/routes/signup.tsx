...
export const Route = createFileRoute("/signup")({
  head: () => ({
    meta: [
      { title: "Create account — Lion Store / إنشاء حساب" },
      { name: "description", content: "Create your Lion Store account for the fastest game and app top-ups." },
...
function SignupPage() {
  ...
  const submit = async (e: FormEvent) => {
    e.preventDefault();
    if (password.length < 6) return toast.error(t("كلمة السر 6 حروف على الأقل", "Password must be at least 6 characters"));
    setLoading(true);
    ...
    if (data.session) {
      ...
      toast.success(t("تم إنشاء الحساب! راجع بريدك لتأكيد الإيميل ثم سجل دخول.", "Account created! Check your email to confirm, then sign in."));
      navigate({ to: "/login" });
    }
  };
    ...
    <div dir={dir} className="min-h-screen grid place-items-center px-4 py-10">
      <div className="w-full max-w-md rounded-3xl bg-dark-gradient border-gold shadow-card p-8">
        <div className="text-center mb-6">
          ...
          <h1 className="mt-4 text-2xl font-black text-gold-gradient">{t("إنشاء حساب", "Create account")}</h1>
          <p className="text-sm text-muted-foreground mt-1">{t("انضم لـ Lion Store", "Join Lion Store")}</p>
        </div>

        <form onSubmit={submit} className="space-y-3">
          <div>
            <label className="text-xs font-bold mb-1 block">{t("الاسم بالكامل", "Full name")}</label>
          ...
          <div>
            <label className="text-xs font-bold mb-1 block">{t("رقم الموبايل", "Phone number")}</label>
            <input required value={phone} onChange={(e) => setPhone(e.target.value)} dir="ltr" placeholder="01XXXXXXXXX" maxLength={20} className="w-full rounded-xl bg-secondary/60 border border-border px-4 py-3 focus:outline-none focus:ring-2 focus:ring-gold/50" />
          </div>
          <div>
        ...
        <button onClick={google} className="w-full rounded-xl border border-border bg-card hover:bg-secondary py-3 font-bold flex items-center justify-center gap-2">
          ...
          {t("تسجيل بـ Google", "Sign up with Google")}
        </button>

        <p className="mt-6 text-sm text-center text-muted-foreground">
          {t("لديك حساب بالفعل؟ ", "Already have an account? ")}<Link to="/login" className="text-gold font-bold">{t("دخول", "Sign in")}</Link>
        </p>
      </div>
    </div>
  );
